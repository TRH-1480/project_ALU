...
module CPU(
    ...
    (* dont_touch = "TRUE" *) ALUCA ALUCA1(.x(x),.y(y),.zx(zx),.nx(nx),.zy(zy),.ny(ny),.f(f),
                    .no(no),.out(result),.zr(zr),.ng(ng));
    
    assign result = out_alu;
    
    //MUX制御
    (* dont_touch = "TRUE" *) myMux myMux1(.rs(rs),.f1(f1),.din(result1),.out(result));
    
    assign result1 = out_mux;
    
    //レジスタファイル制御
    (* dont_touch = "TRUE" *) regfile regfile1(.ck(clk),.we(we),.din(result1),.inaddr(inaddr),.dout(result2),.outaddr(outaddr));
    
    assign result2 = out_reg;
    
    //DeMux
   (* dont_touch = "TRUE" *)  myDeMux myDeMux1(.rs(rs),.ck(clk),.f2(f2),.f3(f3),.x(out_x),.y(out_y),.dout(result2));
    assign x = out_x;
    assign y = out_y;
